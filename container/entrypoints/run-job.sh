#!/bin/bash
# Minimal job execution entrypoint for miles_job.
# Sets up CUDA and Megatron paths, then execs the provided command.
set -euo pipefail
...
activate_mlsys_venv() {
    ...
    if [[ -n "${activate_path}" && -f "${activate_path}" ]]; then
        ...
        echo "[run-job] MLSys venv activated: ${VIRTUAL_ENV:-unknown}"
    fi
}
...
# ── Exec command ────────────────────────────────────────────────────────────
if [[ $# -eq 0 ]]; then
    echo "run-job: no command provided" >&2
    exit 1
fi
